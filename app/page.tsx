...
export default function Home() {
  ...
  return <main>
    ...
    <section className="section selected"><div className="section-head"><div><p className="index">04 — SELECTED PIECES</p><h2>In the <em>spotlight.</em></h2></div><Link className="text-link" href="/products">View all products <span>→</span></Link></div><div className="product-grid">{featured.map((product, i) => <Link className={`product-card product-${i}`} href={`/products/${product.slug}`} key={product.slug}><div className="product-image"><Image src={image(product.images[0])} alt={product.name} fill sizes="(max-width: 700px) 100vw, 50vw" className="contain" /></div><p>{product.type}</p><h3>{product.name}</h3><span>View piece →</span></Link>)}</div></section>
    ...
    <section className="archive-section section"><div><p className="index">06 — SERVOMUTO ARCHIVE</p><h2>Spaces, stories<br />and <em>encounters.</em></h2></div><div className="archive-list">{archiveEntries.slice(0,3).map(entry => <Link key={entry.year + entry.title} href="/archive"><span>{entry.year}</span><strong>{entry.title}</strong><i>{entry.location} →</i></Link>)}</div></section>
